...
<form name="frmcog" method="post" action="/jui/callMessage.so?csrftoken=${csrftoken}"  class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone);">
	...
	<input type="hidden" name="oprID" value="<sc:fmt type='crypto' value='bdssParameter'/>"/>
	<input type="hidden" name="actions" value="<sc:fmt type='crypto' value='importSysParam'/>"/>
...
function downloadTemplate(){	
	var url = '/download?sysName=<sc:fmt value='funcpub' type='crypto'/>&oprID=<sc:fmt value='bdssParameter' type='crypto'/>&actions=<sc:fmt value='downLoadSysParamTemplate' type='crypto'/>';
