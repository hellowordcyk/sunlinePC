...
<form name="frmcog" method="post" action="/jui/callMessage.so?csrftoken=${csrftoken}"  class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone);">
	...
	<input type="hidden" name="oprID" value="<sc:fmt type='crypto' value='SysPubArea'/>"/>
	<input type="hidden" name="actions" value="<sc:fmt type='crypto' value='area_import'/>"/>
...
function downloadTemplate(){	
	var url = '/download?sysName=<sc:fmt value='funcpub' type='crypto'/>&oprID=<sc:fmt value='SysPubArea' type='crypto'/>&actions=<sc:fmt value='downLoadAreaTemplate' type='crypto'/>';
