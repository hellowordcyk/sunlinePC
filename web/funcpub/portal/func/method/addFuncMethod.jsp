...
<form method="post" action="/httpprocesserservlet" class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone)">
	...
	<input type="hidden" name="actions" value="<sc:fmt value='addFuncMethod' type='crypto'/>"/>
		...
		<div class="pageFormContent">
			<table class="form-table" cellpadding="0" cellspacing="0" >
				...
				<tr>
                    ...
                    <td class="form-value">
                        <sc:text name="methodCode" validate="required" />
                    </td>
				</tr>
				<tr>
                    <td class="form-label"><span class="redmust">*</span>功能名称</td>
                    <td class="form-value"><sc:text name="methodName" validate="required"/></td>
                </tr>
				<tr>
					<td class="form-label"><span class="redmust">*</span>所属子系统</td>
					<td class="form-value">
						<c:if test="${param.pType eq 'menu' }">
							<input type="hidden" name="methodSystemid" value="${jrafrpu.rspPkg.rspRcdDataMaps[0].subsysid}"/>
							<sc:optd name="subsysid" type="subsys"  value="${jrafrpu.rspPkg.rspRcdDataMaps[0].subsysid}"/>
						</c:if>
						<c:if test="${param.pType eq 'page' }">
							<input type="hidden" name="methodSystemid" value="${jrafrpu.rspPkg.rspRcdDataMaps[0].pageSubsysid}"/>
				...
				<tr>
					...
					<td class="form-value"><sc:textarea name="methodDesc" rows="3" cols="60" style="resize:none;"/></td>
				</tr>
			</table>
		</div>
	</div>
