...
<form name="frmcog" method="post" action="/jui/callMessage.so?csrftoken=${csrftoken}"  class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone);">
	...
	<input type="hidden" name="oprID" value="<sc:fmt type='crypto' value='sysFuncPageActor'/>"/>
	<input type="hidden" name="actions" value="<sc:fmt type='crypto' value='sysFunc_import'/>"/>
...
function downloadTemplate(){	
	var url = '/download?sysName=<sc:fmt value='funcpub' type='crypto'/>&oprID=<sc:fmt value='sysFuncPageActor' type='crypto'/>&actions=<sc:fmt value='downLoadSysFuncTemplate' type='crypto'/>';
