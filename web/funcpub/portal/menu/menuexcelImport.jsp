...
<form name="frmcog" method="post" action="/jui/callMessage.so?csrftoken=${csrftoken}"  class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone);">
	...
	<input type="hidden" name="oprID" value="<sc:fmt type='crypto' value='funcpub-menumanager'/>"/>
	<input type="hidden" name="actions" value="<sc:fmt type='crypto' value='getMenuImport'/>"/>
...
function downloadTemplate(){	
	var url = '/download?sysName=<sc:fmt value='funcpub' type='crypto'/>&oprID=<sc:fmt value='funcpub-menumanager' type='crypto'/>&actions=<sc:fmt value='getMenuTemplate' type='crypto'/>';
