...
<div class="pageContent">
    ...
    <div class="window-left-box" style="width:28%;" layoutH="130">
	    <ul id="deptTreeUnFilter" class="ztree"></ul>
	</div>
...
var deptTreeSetting = {
	...
	async: {
		enable: true,// 是否异步
		url: "/xmlprocesserservlet?sysName=<sc:fmt value='funcpub' type='crypto'/>&oprID=<sc:fmt value='funcpub-deptusermanager-public' type='crypto'/>&actions=<sc:fmt value='getDeptTreeUnFilter' type='crypto'/>",
		dataType: "xml",// 返回数据类型
		dataFilter : function (treeId, parentNode, responseData){// 数据拦截器，可以将返回的数据组装成你需要的样子
				...
				firstRootId = $(responseData).find("DataPacket Response Data firstRootId").text();
...
$(function(){
	...
	var url = "/xmlprocesserservlet?"
		...
		+ "&actions="+'<sc:fmt value='getDeptTreeUnFilter' type='crypto'/>';
	$.ajax({
	   type: "POST",
	   url: url,
	   dataType: "XML",
	...
	//});
	//初始化树
/* 	treeInstance = $.fn.zTree.init($("#deptTreeUnFilter"),deptTreeSetting,treeObj); */
	treeInstance = $.fn.zTree.init($("#deptTreeUnFilter"),deptTreeSetting);
...
function queryUserByDeptCode(deptcode) {
	...
	var url = "/xmlprocesserservlet?"
        + "sysName="+'<sc:fmt value='funcpub' type='crypto'/>'
        + "&oprID="+'<sc:fmt value='funcpub-deptusermanager-public' type='crypto'/>'
        + "&actions="+'<sc:fmt value='queryUserByDeptCodeUnFilter' type='crypto'/>&deptcode='+deptcode;
    $.ajax({
       type: "POST",
       url: url,
       dataType: "XML",
