...
<div class="pageHeader">
	<form id="pagerForm" action="/httpprocesserservlet?<c:out value='${param.filter }' />" method="post" onsubmit="return dialogSearch(this);">
		...
		<input type="hidden" name="forward" value="<sc:fmt type='crypto' value='/funcpub/public/newpublicOptionControls/kpiOptionDialog.jsp' />"/>
		...
		<input type="hidden" id="daoge" name="daoge"/>
       <%-- 规范： 初始化查询必加隐藏表单 --%>
		...
		<div class="searchBar">
			<table class="searchContent" cellpadding="0" cellspacing="0" >
			    <tr>
			    	...
			    	<td class="form-value"><!--<sc:text name="userCode"/>--><input type="text" name="userCode"/></td>
			    	<td class="form-label">指标名称</td>
				    <td class="form-value"><!--<sc:text name="userName"/>--><input type="text" name="userName"/></td>
				    <td class="form-btn">
					    <ul>
					        <li>
...
$(function(){
	$("#daoge",$.pdialog.getCurrent()).val(new Date().toString());
...
function addSelectedNode(kpiCode,kpiName,kpiType){
	...
	if(node == null){
		treeObj.addNodes(null,{id:kpiCode,name:"["+kpiCode+"]"+kpiName,isParent:false,info:{kpiCode:kpiCode,kpiName:kpiName,kpiType:kpiType}});
