 ...
 * 
 * 2017-01-05 Sean:
 *     * add the close event that defined by self and trigger this event while closing
 */
var navTab = {
	...
	_closeTab: function(index, openTabid, isForceClose){
		...
		//added by Sean on 20170105 trigger the "_closeEvent" event
	...
	/**
	 * close tab straightly 
	 * @param tabid the tab's ID that you want to close 
	 * @param openTabid after closing, open one tab by tab's Id
	 */
	closeTabByForce: function(tabid, openTabid){
		if (tabid) {
			var index = this._indexTabId(tabid);
			if (index > 0) { 
				this._closeTab(index, null, true); 
			}
		} else {
	...
	closeAllTab: function(){
		...
		//modified by Sean on 20170107 before closing the tab, call a 'callback' function 
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				var $pagerForm = getPagerFormObjs($panel);//$("#pagerForm", $panel);
	 ...
	 */
	 //added in jraf 初始化“首页”tab
	openTabNoInit: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
	 ...
	 * @param {Object} params: title, data, fresh
	 */
	openTab: function(tabid, url, options, treeid){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			//added by Sean on 20170105 add the close event that defined by self 
