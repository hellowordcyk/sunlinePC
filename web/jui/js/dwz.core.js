...
var DWZ = {
	...
	//added by Sean on 20160919 define the background div's zIndex(mask div)
	maskArgs: {
		dialogZindex: 1000
	},
	...
	loadLogin:function(){
		if ($.pdialog && DWZ._set.loginTitle) {
			$.pdialog.open(DWZ._set.loginUrl, "login", DWZ._set.loginTitle, {mask:true,width:520,height:260,close:false,maxable:false});
		} else {
	...
	ajaxError:function(xhr, ajaxOptions, thrownError){
		//add a judgement to statusCode of timeout by beidao 20160712
		...
		//add a judgement to statusCode of unaccess by beidao 20160715
...
(function($){
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			...
			$.ajax({
				...
				success: function(response){
					...
					} else if(json[DWZ.keys.statusCode]== DWZ.statusCode.ssologin){
		...
		loadUrl: function(url,data,callback){
			...
			//modified by Sean on 20170110 wrap the callback function
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				//modified by Sean on 20170110 change the default parent to the element have the property of layoutH
				...
				if ($this.isTag("table")) {
					//added the position style on 20170125 by Sean
					$this.removeAttr("layoutH").wrap('<div layoutH="'+iLayoutH+'" style="position: relative;overflow:auto;height:'+iH+'px"></div>');
				} else {
	...
	$.extend(String.prototype, {
		...
		replaceTmById:function(_box, resousejson){
			return this.replace(RegExp("({[A-Za-z_]+[A-Za-z0-9_]*})","g"), function($1){
				...
				var value = "";
				if(resousejson){
					value = resousejson[key] || "";
				}else{
					var $parent = _box || navTab.getCurrentPanel() || $.pdialog.getCurrent() || $(document);
					var $input = $parent.find("#"+key);
					value = $input.val() || "";
				}
				//return $input.val()?$input.val():$1;  修改如果$input找不到，返回空字符串，以免页面alert（“请选择信息”），导致不进入后台查询 edit by lihu
				return value;
			});
		},
