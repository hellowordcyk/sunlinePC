		...
		return this.each(function(){
			...
			$this.click(function(event){
				...
				function disabledMonth(){
					//add by yangyuanyuan 20171211 不在最大与最小时间月份选项设置为disabled
				...
				if(!dp.hasDay()){
					//add by yangyuanyuan 20171211
					if(!dp.hasMonth()){
						
						$(setting.main$).addClass('nomonth');
					}else{
						//可以仅仅选择年月
			...
			// Modified By TonyFang,修改为查找紧邻的日历图标
		...
		if(minP == "#"){
			...
			if (minDateObj.length == 1){
				if($(minDateObj, $p).val() != null && $(minDateObj, $p).val() != "" ){
					this.opts.minDate = $(minDateObj, $p).val();
				}else{
		...
		if(maxP=="#"){
			...
			if(maxDateObj && maxDateObj.length == 1){
				if($(maxDateObj, $p).val() != null && $(maxDateObj, $p).val() != "" ){
					this.opts.maxDate = $(maxDateObj, $p).val();
				}else{
