...
(function($){
	...
	$.extend({
		bringBackSuggest: function(args){
			...
			//edited by hzx 20161214 improve algorithm of bringback value by id or name 
			for (var key in args) {
				var name = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
				if(lookuptype=="id"){
					$("#"+name,$box).val(args[key]);
				}else{
			...
			/*$box.find(":input").each(function(){
				...
				//edited by hzx 20161103 bringback value by id
...
/*				if($(this).attr("jf-init")) return false; //如果已经初始化，则直接跳过。
				else $(this).attr("jf-init","true");*/
...
/*					$this.parent("td").hover(function(){
						$this.siblings('.btnLook_clean').show();
					},function(){
						$this.siblings('.btnLook_clean').hide();
					})*/
				})
				
				if(!$this.hasClass("btnLook")){
					
				}else if(siblingsText){
				...
				Jraf.lookupClear($this);  //clean up all values , 20170310
				$this.click(function(event){
					_lookup.$target.push($this); //add by yangyuanyuan
		...
		suggest: function(){
			...
			return this.each(function(){
				...
				function _show(event){
					...
					_lookup.$target.push($input); //add by yangyuanyuan
		...
		selectedTodo: function(){
			...
			//added by Longjian on 2017-01-04 data privilege control
			...
			return this.each(function(){
				var $this = $(this);
				var selectedIds = $this.attr("rel") || $this.attr("group") || "ids";   //selectedTodo����group
				var postType = $this.attr("postType") || "map";
				
				$this.click(function(event){
					var targetType = $this.attr("targetType");
					var pkvalArr = _getPkvals(selectedIds, targetType);
					var ids = pkvalArr[0].vals;  //_getIds(selectedIds, targetType);
					...
					var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));   //selectedTodo �滻url
