...
{try{
	...
	{	if(selectedNode!=null){
	        ...
	        //兼容处理
	        selectedNode.label.setAttribute("class", "tree-undoselect");
	        selectedNode.label.setAttribute("className", "tree-undoselect");
	    }
	    //
	    srcNode.label.setAttribute("class", "tree-doselect");
	    srcNode.label.setAttribute("className", "tree-doselect");
	...
	{	
	    if(srcNode==selectedNode){
	        srcNode.label.setAttribute("class", "tree-dofocus");
	        srcNode.label.setAttribute("className", "tree-dofocus");
	        //srcNode.label.style.background="highlight";srcNode.label.style.color="highlighttext";
	    }
		activeNode=srcNode
	}
	...
	{	if(srcNode==selectedNode){
	        srcNode.label.setAttribute("class", "tree-doblur");
	        srcNode.label.setAttribute("className", "tree-doblur");
	        //srcNode.label.style.background="buttonface";srcNode.label.style.color="windowtext";
	    }
		activeNode=null
	}
	var doMouseover=function(srcNode)
	{	
	    if(srcNode!=selectedNode){
	        srcNode.label.setAttribute("class", "tree-domouseover");
	        srcNode.label.setAttribute("className", "tree-domouseover");
	    }
	}
	var doMouseout=function(srcNode)
	{	
	    if(srcNode!=selectedNode){
	        srcNode.label.setAttribute("class", "tree-domouseout");
	        srcNode.label.setAttribute("className", "tree-domouseout");
	    }
	}
	...
	var addNode=function(toNode,relation,text,key,ico,exeCategory,exeArg)
	{
		...
		//modified by yx on 2012-11-21
        container.setAttribute("class", "tree-container");
		container.setAttribute("className", "tree-container");
		//container.style.cssText="margin:0px;padding:0px;white-space:nowrap;"
		with(nbody)
		{	insertAdjacentElement("beforeEnd",label)
		    //modified by yx on 2012-11-21
	        setAttribute("class", "tree-body");
		    setAttribute("className", "tree-body");
			//style.cssText="cursor:default;text-align:left;color:black;font-size:9pt;margin:0px;height: 20px;"
		...
		label.setAttribute("className", "tree-label"); 
		if (labelStyle!=null) {
		...
		with(label)
		{
			...
			oncontextmenu=function(){run(node.select);if(run(node.oncontextmenu))if(run(tree.oncontextmenu,node))run(node.rightexecute);}//节点右击实现
			...
			onmouseover=function(){if(event.fromElement!=node.icon)if(run(node.onmouseover)){if(run(tree.onmouseover,node))doMouseover(node);}}
			//onmouseout=function(){if(event.toElement!=node.icon)if(run(node.onmouseout))run(tree.onmouseout,node);}
			onmouseout=function(){if(event.toElement!=node.icon)if(run(node.onmouseout)){if(run(tree.onmouseout,node))doMouseout(node);}}
		...
		{
			onclick=function(){run(node.select);if(run(node.onclick))if(run(tree.onclick,node))run(node.execute);}
			oncontextmenu=function(){run(node.select);if(run(node.oncontextmenu))if(run(tree.oncontextmenu,node))run(node.rightexecute);}
			ondblclick=function(){if(run(node.ondblclick))run(tree.ondblclick,node)}
			onmouseover=function(){if(event.fromElement!=node.icon)if(run(node.onmouseover))run(tree.onmouseover,node)}
			onmouseout=function(){if(event.toElement!=node.icon)if(run(node.onmouseout))run(tree.onmouseout,node);}
			onmousemove=function(){if(run(node.onmousemove))run(tree.onmousemove,node)}
		...
		node.onclick=node.oncontextmenu=node.ondblclick=node.onmouseover=node.onmouseout=node.onmousemove=node.onmousedown=node.onmouseup=node.onkeypress=node.onkeydown=node.onkeyup=node.oncollapse=node.onexpand=node.onfocus=node.onblur=node.onselect=new Function("return true");
		...
		node.rightexecute=new Function()
		exeCategory=exeCategory==null?"":exeCategory.toLowerCase()
		switch(exeCategory)
		{	case "expand":
				node.execute=node.expand;node.rightexecute=node.expand;break;
				
			case "js":
				if(typeof(exeArg)!="string")break;node.execute=function(){eval(exeArg);};node.rightexecute=function(){eval(exeArg);};break;
			default:
				if(exeCategory.indexOf("url")!=-1)
				{	if(typeof(exeArg)!="string"){break;}
					if(exeCategory.indexOf("url:")!=-1)
					{	var tar=exeCategory.replace("url:","");node.execute=function(){open(exeArg,tar);} ;node.rightexecute=function(){open(exeArg,tar);}
					}else {	node.execute=function(){open(exeArg,tree.target);} ;node.rightexecute=function(){open(exeArg,tree.target);}}
					exeCategory="url";
				}else {node.execute=node.expand;node.rightexecute=node.expand;exeCategory="expand";}
		}
	...
	//added by yx on 2012-11-21
	this.body.setAttribute("class", "tree-main");
	this.body.setAttribute("className", "tree-main");
	...
	this.onclick=this.oncontextmenu=this.ondblclick=this.onmousemove=this.onmouseover=this.onmouseout=this.onmouseup=this.onmousedown=this.onkeypress=this.onkeydown=this.onkeyup=this.oncollapse=this.onexpand=this.afteradd=this.onblur=this.onfocus=this.onselect=new Function("return true;");
	this.add=addNode
