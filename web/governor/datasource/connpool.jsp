...
<table class="form-table" cellpadding="0" cellspacing="0" >
        <tr>
	        <td class="form-label"><span style="color: red;">*</span>数据库类型</td>
	        <td class="form-value" >
	        ...
	        <td class="form-label"><span style="color: red;">*</span>数据库驱动</td>
	        <td class="form-value">
	        	<select id="dbDriver" style="width: 152px;"></select>
	        </td>
       	</tr>
        <tr>
        	<td class="form-label"><span style="color: red;">*</span>服务器地址</td>
       		<td class="form-value"><input id="address" class="required" req="1" type="text" onKeyUp="ganged()" value='${sourceInfo.serverAddr }'/></td>
	        <td class="form-label"><span style="color: red;">*</span>数据库名称</td>
	        <td class="form-value"><input id="dbname" class="required" req="1" type="text" onKeyUp="ganged()" value='${sourceInfo.dbName }'/></td>
	    </tr>
        <tr>
        	<td class="form-label"><span style="color: red;">*</span>用户名</td>
	        <td class="form-value"><input id="username" class="required" req="1" type="text" onKeyUp="changesavebtn('true')" value='${sourceInfo.userName }' /></td>
        	<td class="form-label"><span style="color: red;">*</span>密码</td>
	        <td class="form-value"><input id="password" class="required" req="1" type="password" onKeyUp="changesavebtn('true')" style="width:147px" /></td>
        </tr>
        <tr>
        	<td class="form-label"><span style="color: red;">*</span>驱动URL</td>
        	<td class="form-value" colspan="3" style="padding:8px 0 8px 10px;">
        		<textarea id="driverURL" class="inputarea required"" name="driverURL" req="1" onKeyUp="changesavebtn('true')">${sourceInfo.driverURL }</textarea>
        	</td>
        </tr>
        <tr>
        	<td class="form-label"><span style="color: red;">*</span>测试语句</td>
        	<td class="from-value" colspan="3" style="padding:8px 0 8px 10px;">
        		<textarea id="testSQL" name="testSQL" class="inputarea required" req="1" onKeyUp="changesavebtn('true')">${sourceInfo.testSQL }</textarea>
        	</td>
        </tr>
         <tr>
        	<td class="form-label"><span style="color: red;">*</span>是否启用sql监控</td>
        	<td >
        		<input id="yes_is_JDBmonitor" name="isJDBmonitor" type="radio" value="true" <c:if test="${sourceInfo.isJDBmonitor=='true' }">checked="checked"</c:if> disabled />是&nbsp;&nbsp;
        		<input id="on_is_JDBmonitor" name="isJDBmonitor" type="radio" value="false" <c:if test="${sourceInfo.isJDBmonitor=='false' }">checked="checked"</c:if> disabled />否
        	</td>
        </tr>
        <tr>
        	<td class="form-label">使用前测试</td>
        	<td >
        		<input id="yes_before_use" name="beforeUse" type="radio" value="true" <c:if test="${sourceInfo.testBeforeUse=='true' }">checked="checked"</c:if> disabled="disabled"/>是&nbsp;&nbsp;
        		<input id="on_before_use" name="beforeUse" type="radio" value="false" <c:if test="${sourceInfo.testBeforeUse=='false' }">checked="checked"</c:if> disabled/>否
        	</td>
        	<td class="form-label">使用后测试</td>
        	<td >
        		<input id="yes_after_use" name="afterUse" type="radio" value="true" <c:if test="${sourceInfo.testAfterUse=='true' }">checked="checked"</c:if> disabled="disabled "/>是&nbsp;&nbsp;
        		<input id="on_after_use" name="afterUse" type="radio" value="false" <c:if test="${sourceInfo.testAfterUse=='false' }">checked="checked"</c:if> disabled/>否
        	</td>
        </tr>
        <tr>
        	<td class="form-label">详细信息设置</td>
        	<td >
        		<input id="yes_verbose" name="verbose" type="radio" value="true" <c:if test="${sourceInfo.verbose=='true' }">checked="checked"</c:if> disabled="disabled" />是&nbsp;&nbsp;
        		<input id="on_verbose" name="verbose" type="radio" value="false" <c:if test="${sourceInfo.verbose=='false' }">checked="checked"</c:if> disabled="disabled" />否
        	</td>
        	<td class="form-label">SQL语句是否被log记录</td>
        	<td >
        		<input id="yes_trace" name="trace" type="radio" value="true" <c:if test="${sourceInfo.trace=='true' }">checked="checked"</c:if> disabled="disabled" />是&nbsp;&nbsp;
        		<input id="on_trace" name="trace" type="radio" value="false" <c:if test="${sourceInfo.trace=='false' }">checked="checked"</c:if> disabled="disabled" />否
        	</td>
        </tr>
        <tr>
        	<td class="form-label">睡眠状态时间</td>
        	<td class="form-value"><input id="sleep_time" type="text"  value='${sourceInfo.sleepTime }' disabled="disabled" /></td>
        	<td class="form-label">线程最大寿命</td>
	        <td class="form-value"><input id="max_conn_lifetime" type="text"  value='${sourceInfo.maxConnLifetime }' disabled="disabled" /></td>
        </tr>
        <tr>
        	<td class="form-label">最大的数据库连接数</td>
	        <td class="form-value"><input id="maxconn" type="text"  value='${sourceInfo.maxConn }' disabled="disabled" /></td>
        	<td class="form-label">最小的数据库连接数</td>
        	<td class="form-value"><input id="minconn" type="text" value='${sourceInfo.minConn }' disabled="disabled" /></td>
        </tr>
        <tr>
        	<td class="form-label">一次建立的最大连接数</td>
	        <td class="form-value"><input id="build_throttle" type="text"  value='${sourceInfo.buildThrottle }' disabled="disabled" /></td>
        	<td class="form-label">连接池状态：</td>
        	<td class="form-value"><input id="started_threshold" type="text" value='${sourceInfo.startedThreshold }' disabled="disabled" /></td>
        </tr>
        <tr>
        	<td class="form-label">线程的活动时间</td>
        	<td class="form-value"><input id="max_active_time" type="text" value='${sourceInfo.maxActiveTime }' disabled="disabled" /></td>
        	<td class="form-label">空闲连接数量</td>
	        <td class="form-value"><input id="prototype_count" type="text"  value='${sourceInfo.prototypeCount }' disabled="disabled" /></td>
        </tr>
</table>
...
$("select[name=dbType] option[value="+h_dbtype+"]").attr("selected", true);
...
function func(c){
    ...
    if(ret==0){
      ...
      changesavebtn(false);
    	...
    	alertMsg.correct("参数保存成功！正在重启服务");
    	//加载数据库配置获取连接
    	setTimeout(function(){
    		reload();
    	},1000);
...
function commonGanged(dbtype,address,dbname) {
	if (dbtype=="mysql") {
		...
		document.getElementById("testSQL").value="select 1 from sysibm.sysdummy1";
	} else if (dbtype=="mssql") {
		addOption(dbtype);
		document.getElementById("driverURL").value="jdbc:jtds:sqlserver://"+address+":1433;databaseName=" + dbname;
		document.getElementById("testSQL").value="select getdate() ";
	} else if (dbtype=="hsql") { 
		addOption(dbtype);
		...
		document.getElementById("testSQL").value="select now()";
		...
		document.getElementById("testSQL").value="SELECT current FROM sysmaster:sysshmvals;";
...
function reload(){
	var url="/reloadConfig";
  	$("body").jrafAjaxSend();
	m_Ajax.sendRequest("POST","/reloadConfig",null,funS);
}
function funS(c){
    var ret = c.text;
