...
<sc:fieldset name="子系统修改/删除">
	<form method="post" action="/httpprocesserservlet">
		...
		<input type="hidden" name="actions" value="<sc:fmt value='updateSubSys' type='crypto'/>">
		<input type="hidden" name="forward" value="<sc:fmt type='crypto' value='/pcmc/sm/updateSubSysList.jsp' />">
		<!-- 子系统列表面板 -->
		<table width="100%" cellpadding="0" cellspacing="0" class="form-table">
			...
			<tr>
				...
				%>
				<td align=center><DIV onclick="sel(<%=i%>,'<%=recordElement.getChildTextTrim("subsysid")%>','<%=recordElement.getChildTextTrim("shortname")%>','<%=recordElement.getChildTextTrim("enname")%>', '<%=recordElement.getChildTextTrim("cnname")%>','<%=recordElement.getChildTextTrim("imgurl")%>','<%=recordElement.getChildTextTrim("linkurl")%>','<%=recordElement.getChildTextTrim("pubinfourl")%>','<%=recordElement.getChildText("orderidx")%>');" ondblclick="document.location.href = '/httpprocesserservlet?sysName=<sc:fmt value='pcmc' type='crypto'/>&oprID=<sc:fmt value='sm_query' type='crypto'/>&actions=<sc:fmt value='getMenuListBySubSysID' type='crypto'/>&subsysid=<%=recordElement.getChildTextTrim("subsysid")%>&cnname=<%=recordElement.getChildTextTrim("cnname")%>&level=0&forward=<sc:fmt value='/pcmc/sm/updateMenuList.jsp' type='crypto'/>';"  class=coolButton style='cursor:hand;height:34px;width:34px;margin:2px;'><img src='/common/images/pcmc/subsystem.gif'></div>
				<font id=link1><%=recordElement.getChildTextTrim("cnname")%></font></td>
				<%
				        }
		...
		<table width="100%" cellpadding="0" cellspacing="0" class="form-table">
			...
			<tr><th colspan="10">修改/删除子系统</th></tr>
			<tr>
...
function save(){
	document.forms[0].actions.value='<sc:fmt value='updateSubSys' type='crypto'/>';
...
function doDelete(){
	document.forms[0].actions.value='<sc:fmt value='deleteSubSys' type='crypto'/>';
	if (confirm('你确定要删除此子系统？')) document.forms[0].submit();
}

function sel(x,subsysid,shortname,enname,cnname,imgurl,linkurl,pubinfourl,orderidx){
  ...
  document.forms[0].shortname.value = shortname;
  document.forms[0].enname.value = enname;
  document.forms[0].cnname.value = cnname;
  document.forms[0].imgurl.value = imgurl;
  document.forms[0].linkurl.value = linkurl;
  document.forms[0].pubinfourl.value = pubinfourl;
  document.forms[0].orderidx.value = orderidx;
  return true;
}
