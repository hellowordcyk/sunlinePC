...
<sc:form name="query_form" action="/pcmc/pm/query_pcmcinfo.so" method="post" sysName="pcmc" oprID="info" actions="queryPcmcInfoByLogUser" attributesText="onSubmit='return checkForm(this);'">
...
<table class="form-table" border="0" width="100%" cellspacing="0" cellpadding="0">
    ...
    <tr>
        <sc:text dsp="td" name="title" dspName="公告标题" req="0"/> 
         <td class="form-label">是否已读</td>
         <td class="form-value">
          <sc:select dspName="是否已读" name="isread" type="bdss" key="bdss,isread" includeTitle="false" default="0" nullOption="---请选择---">
          </sc:select>
         </td>
    </tr>
   <tr>
...
<sc:form name="page_form" method="post" action="/pcmc/pm/query_pcmcinfo.so" sysName="pcmc" oprID="info" actions="queryPcmcInfoByLogUser" attributesText="onSubmit='return checkForm(this);'">
<sc:hidden name="ispriv"/>
...
<sc:hidden name="username" />  
    <display:table uid="record" name="jrafrpu.rspPkg.rspRcdDataMaps" class="list-table" requestURI="/httpprocesserservlet"
              sort="list">    
    <display:column title="是否已读" style="width: 80px;">
        <sc:optd type="bdss" key="bdss,isread" value="${record.isread}"/>
    </display:column>
...
function doSearch(){
    ...
    formObj.actions.value='<sc:fmt type="crypto" value="queryPcmcInfoByLogUser"/>';
...
{
    //var url = '/httpprocesserservlet?sysName=<sc:fmt type="crypto" value="pcmc"/>&oprID=<sc:fmt type="crypto" value="info"/>&actions=<sc:fmt type="crypto" value="queryInfoContent"/>&forward=<sc:fmt value='/pcmc/pm/showInfoContent.jsp' type='crypto'/>&infoid='+infoid;
