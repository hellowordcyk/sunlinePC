...
<head>
	...
	<title>新增系统权限分类</title>
</head>
<body>
<sc:form name="form" action="/xmlprocesserservlet" method="post" sysName="pcmc" oprID="privActor" actions="addPrvBase">
	<table width="100%" cellpadding="0" cellspacing="0" class="form-table">
		...
		<tr>
		    ...
		    <td class="form-value">
		    	<sc:text name="bscode" dspName="系统分类代码" req="1" attributesText="id='bscode' maxLength='32' readOnly" />
		    	<!-- <img src="/common/images/tree/hfile.gif" border=0 style="CURSOR: hand" onClick="getBsCode()"> -->
		    	<input type="button" id="#" name="bscode_search" onclick="getBsCode();" src="" class="search" />  
		    </td>
		</tr>
		<tr>
			<sc:text name="bsname" dspName="系统分类名称" dsp="td" req="1" attributesText="maxLength='64'" />
		</tr>
		<tr>
			<sc:select name="status" dspName="权限分类状态" type="dict" key="pcmc,status" default="1" includeTitle="false" dsp="td" req="1" />
		</tr>
		<tr>
			<td class="form-label"><font color="red">*</font>系统管理员代码 ：</td>
		    <td class="form-value">
		    	<sc:text name="sysopr" dspName="系统管理员代码"  req="1" attributesText="id='sysopr' maxLength='32' readOnly" />
		    	<img src="/common/images/pic01/sel_person.gif" border=0 style="CURSOR: hand" onClick="getUserInfo()">
		    </td>
		</tr>
		<tr>
			<sc:text name="operna" dspName="系统管理员名称" dsp="td" attributesText="id='operna' maxLength='64' readOnly" />
		</tr>
	</table>
	<div style="width:100%" align="center" class="page-bottom">
        <sc:button value="保存" onclick="addPrvBase()"/>
...
function addPrvBase() {
	if (checkForm(document.form)){
		ajaxSubmit("form", "新增系统权限分类成功！", "新增系统权限分类失败！");
	}
}
/** 退出 */
function quit() {
